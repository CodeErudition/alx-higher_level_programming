...
class TestBase(unittest.TestCase):
    ...
    def test_Base_nb_objects_private(self):
        """Tests if nb_objects is private class attribute."""
        self.assertTrue(hasattr(Base, "_Base__nb_objects"))

    def test_Base_nb_objects_initialized(self):
    ...
    def test_Base_constructor_args_2(self):
    ...
    def test_Base_custom_id_int(self):
        """Tests custom int id."""
        i = 98
        b = Base(i)
        self.assertEqual(b.id, i)

    def test_Base_custom_id_str(self):
        """Tests custom int id."""
        i = "FooBar"
        b = Base(i)
        self.assertEqual(b.id, i)

    def test_Base_id_keyword(self):
    ...
    def test_Base_to_json_string(self):
        ...
        d = [{'x': 101, 'y': 30123, 'width': 512321, 'id': 622244, 'height': 44340}]
        self.assertEqual(len(Base.to_json_string(d)), len(str(d)))
        d = [{'x': 1, 'y': 2, 'width': 3, 'id': 4, 'height': 5}]
        self.assertEqual(len(Base.to_json_string(d)), len(str(d)))
        d = [{"codeerudition": 989898}]
        self.assertEqual(Base.to_json_string(d), '[{"codeerudition": 989898}]')

        d = [{"codeerudition": 989898}, {"abc": 123}, {"HI": 0}]
        self.assertEqual(Base.to_json_string(d), '[{"codeerudition": 989898}, {"abc": 123}, {"HI": 0}]')

        d = [{'x': 1, 'y': 2, 'width': 3, 'id': 4, 'height': 5}, {'x': 101, 'y': 20123, 'width': 312321, 'id': 522244, 'height': 34340}]
        self.assertEqual(len(Base.to_json_string(d)), len(str(d)))
        d = [{}]
    ...
    def test_Base_from_json_string(self):
        ...
        s = '[{"x": 1, "y": 2, "width": 3, "id": 4, "height": 5}, {"x": 101, "y": 20123, "width": 312321, "id": 522244, "height": 34340}]'
        d = [{'x': 1, 'y': 2, 'width': 3, 'id': 4, 'height': 5}, {'x': 101, 'y': 20123, 'width': 312321, 'id': 522244, 'height': 34340}]
        ...
        d = [{"codeerudition": 989898}, {"abc": 123}, {"HI": 0}]
        s = '[{"codeerudition": 989898}, {"abc": 123}, {"HI": 0}]'
        self.assertEqual(Base.from_json_string(s), d)

        d = [{"codeerudition": 989898}]
        s = '[{"codeerudition": 989898}]'
        self.assertEqual(Base.from_json_string(s), d)
        ...
        list_in = [{'id': 89, 'width': 10, 'height': 4}, {'id': 7, 'width': 1, 'height': 7}]
